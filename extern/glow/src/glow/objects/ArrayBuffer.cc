...
BoundArrayBuffer* ArrayBuffer::getCurrentBuffer()
{
...
void ArrayBuffer::defineAttribute(const ArrayBufferAttribute& a)
{
    ...
    if (a.fixedStride > 0)
    {
        ...
        {
            error() << "Attribute requires fixed stride of " << a.fixedStride << " but stride is already set to "
                    << mStride << " " << to_string(this);
...
bool BoundArrayBuffer::isCurrent() const
{
    GLOW_RUNTIME_ASSERT(sCurrentBuffer == this,
                        "Currently bound AB does NOT match represented buffer " << to_string(buffer), return false);
    return true;
}

void BoundArrayBuffer::setData(size_t sizeInBytes, const void* data, GLenum usage)
{
    implSetData(sizeInBytes, data, 0, usage);
}
