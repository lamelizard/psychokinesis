...
Texture3D::Texture3D (Texture3D::Shape const& shape)
  : Texture(GL_TEXTURE_3D, GL_TEXTURE_BINDING_3D, shape.format)
{
...
SharedTexture3D Texture3D::create(Texture3D::Shape const& shape)
{
    auto tex = std::make_shared<Texture3D>(shape);
    return tex;
}
...
SharedTexture3D Texture3D::createStorageImmutable(Shape const& shape)
{
    GLOW_RUNTIME_ASSERT(shape.immutableMipmaps >= 0, "Attemped to create Texture3D with negative amount of Mipmaps", return nullptr);
    auto tex = std::make_shared<Texture3D>(shape);
