...
BoundAtomicCounterBuffer* AtomicCounterBuffer::getCurrentBuffer()
{
...
bool BoundAtomicCounterBuffer::isCurrent() const
{
    GLOW_RUNTIME_ASSERT(sCurrentBuffer == this,
                        "Currently bound UB does NOT match represented buffer " << to_string(buffer), return false);
    return true;
}
...
void BoundAtomicCounterBuffer::setCounters(uint32_t value, int count, GLenum usage)
{
...
int BoundAtomicCounterBuffer::getCounterSize() const
{
