...
public: // getter
    ...
    glm::ivec3 getDimensions() const override { return { mWidth, mHeight, 1 }; }
...
public:
    ...
    struct BoundTextureRectangle
    {
        ...
        void setData(GLenum internalFormat, int width, int height, std::vector<DataT> const& data)
        {
        ...
        void setData(GLenum internalFormat, int width, int height, const DataT(&data)[N])
        {
            if (N != width * height)
            {
        ...
        template <typename DataT>
        void setSubData(int x, int y, int width, int height, std::vector<DataT> const& data)
        {
            if ((int)data.size() != width * height)
            {
                error() << "Texture size is " << width << " x " << height << " = " << width * height << " but " << data.size()
        ...
        template <typename DataT, std::size_t N>
        void setSubData(int x, int y, int width, int height, const DataT(&data)[N])
        {
            if (N != width * height)
            {
                error() << "Texture size is " << width << " x " << height << " = " << width * height << " but " << N
...
public: // static construction
    ...
    static SharedTextureRectangle create(SizeT size, GLenum internalFormat = GL_RGBA) { return create(size.x, size.y, internalFormat); }
    ...
    static SharedTextureRectangle createStorageImmutable(SizeT size, GLenum internalFormat) { return createStorageImmutable(size.x, size.y, internalFormat); }
