...
TextureRectangle::TextureRectangle (TextureRectangle::Shape const& shape)
  : Texture(GL_TEXTURE_RECTANGLE, GL_TEXTURE_BINDING_RECTANGLE, shape.format)
{
...
SharedTextureRectangle TextureRectangle::create(TextureRectangle::Shape const& shape)
{
    auto tex = std::make_shared<TextureRectangle>(shape);
    return tex;
}
...
SharedTextureRectangle TextureRectangle::createStorageImmutable(Shape const& shape)
{
    auto tex = std::make_shared<TextureRectangle>(shape);
    auto& size = shape.size;
    tex->bind().makeStorageImmutable(size.x, size.y, shape.format);
    return tex;
}
