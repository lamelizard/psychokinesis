...
Texture2DMultisampleArray::Texture2DMultisampleArray (Texture2DMultisampleArray::Shape const& shape)
  : Texture(GL_TEXTURE_2D_MULTISAMPLE_ARRAY, GL_TEXTURE_BINDING_2D_MULTISAMPLE_ARRAY, shape.format)
{
...
SharedTexture2DMultisampleArray Texture2DMultisampleArray::create(Texture2DMultisampleArray::Shape const& shape)
{
    auto tex = std::make_shared<Texture2DMultisampleArray>(shape);
    return tex;
}
...
void Texture2DMultisampleArray::BoundTexture2DMultisampleArray::resize(int width, int height, int layers, int samples, GLboolean fixedSamples)
{
    if (!isCurrent())
        return;
    checkValidGLOW();
    ...
    glTexImage3DMultisample(texture->mTarget, samples, texture->mInternalFormat, width, height, layers, fixedSamples);
...
void Texture2DMultisampleArray::BoundTexture2DMultisampleArray::setMultisampling(int samples, GLboolean fixedSamples)
{
    if (!isCurrent())
        return;
    checkValidGLOW();

    glTexImage3DMultisample(texture->mTarget, samples, texture->mInternalFormat, texture->mWidth, texture->mHeight, texture->mLayers, fixedSamples);
    texture->mSampleAmount = samples;
    texture->mFixedSamples = fixedSamples;
}
