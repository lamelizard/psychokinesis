...
TextureCubeMap::TextureCubeMap (TextureCubeMap::Shape const& shape)
  : Texture(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_BINDING_CUBE_MAP, shape.format)
{
...
SharedTextureCubeMap TextureCubeMap::create(TextureCubeMap::Shape const& shape)
{
    auto tex = std::make_shared<TextureCubeMap>(shape);
    return tex;
}
...
SharedTextureCubeMap TextureCubeMap::createStorageImmutable(Shape const& shape)
{
    GLOW_RUNTIME_ASSERT(shape.immutableMipmaps >= 0, "Attemped to create TextureCubeMap with negative amount of Mipmaps", return nullptr);
    auto tex = std::make_shared<TextureCubeMap>(shape);
    auto& size = shape.size;
    tex->bind().makeStorageImmutable(size.x, size.y, shape.format, shape.immutableMipmaps);
    return tex;
}
