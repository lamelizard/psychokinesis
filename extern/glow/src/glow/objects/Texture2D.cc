...
Texture2D::Texture2D (Texture2D::Shape const& shape)
  : Texture(GL_TEXTURE_2D, GL_TEXTURE_BINDING_2D, shape.format)
{
...
SharedTexture2D Texture2D::create(Texture2D::Shape const& shape)
{
    auto tex = std::make_shared<Texture2D>(shape);
    return tex;
}
...
SharedTexture2D Texture2D::createStorageImmutable(Shape const& shape)
{
    GLOW_RUNTIME_ASSERT(shape.immutableMipmaps >= 0, "Attemped to create Texture2D with negative amount of Mipmaps", return nullptr);
    auto tex = std::make_shared<Texture2D>(shape);
