...
Texture1DArray::Texture1DArray (Texture1DArray::Shape const& shape)
  : Texture(GL_TEXTURE_1D_ARRAY, GL_TEXTURE_BINDING_1D_ARRAY, shape.format)
{
...
SharedTexture1DArray Texture1DArray::create(Texture1DArray::Shape const& shape)
{
    auto tex = std::make_shared<Texture1DArray>(shape);
    return tex;
}
...
SharedTexture1DArray Texture1DArray::createStorageImmutable(Shape const& shape)
{
    GLOW_RUNTIME_ASSERT(shape.immutableMipmaps >= 0, "Attemped to create Texture1DArray with negative amount of Mipmaps", return nullptr);
    auto tex = std::make_shared<Texture1DArray>(shape);
