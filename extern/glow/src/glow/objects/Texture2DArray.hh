...
public: // getter
    ...
    glm::ivec3 getDimensions() const override { return { mWidth, mHeight, mLayers }; }
...
public:
    ...
    struct BoundTexture2DArray
    {
        ...
        void resize(SizeT size) { resize(size.x, size.y, size.z); }
...
public:
    ...
    Texture2DArray(Shape const& shape);

    /// Binds this texture.
    /// Unbinding is done when the returned object runs out of scope.
    GLOW_NODISCARD BoundTexture2DArray bind() { return {this}; }
public: // static construction
    ...
    /// Creates an array of 2D textures from Shape
    static SharedTexture2DArray create(Shape const& shape);
    static SharedTexture2DArray create(SizeT size, GLenum internalFormat = GL_RGBA) { return create(size.x, size.y, size.z, internalFormat); }
    ...
    static SharedTexture2DArray createStorageImmutable(SizeT size, GLenum internalFormat, int mipmapLevels = 0) { return createStorageImmutable(size.x, size.y, size.z, internalFormat, mipmapLevels); }
    static SharedTexture2DArray createStorageImmutable(Shape const& shape);
