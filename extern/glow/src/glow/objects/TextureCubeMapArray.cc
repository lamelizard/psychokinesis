...
TextureCubeMapArray::TextureCubeMapArray (TextureCubeMapArray::Shape const& shape)
  : Texture(GL_TEXTURE_CUBE_MAP_ARRAY, GL_TEXTURE_BINDING_CUBE_MAP_ARRAY, shape.format)
{
...
SharedTextureCubeMapArray TextureCubeMapArray::create(TextureCubeMapArray::Shape const& shape)
{
    auto tex = std::make_shared<TextureCubeMapArray>(shape);
    return tex;
}
...
SharedTextureCubeMapArray TextureCubeMapArray::createStorageImmutable(Shape const& shape)
{
    GLOW_RUNTIME_ASSERT(shape.immutableMipmaps >= 0, "Attemped to create TextureCubeMapArray with negative amount of Mipmaps", return nullptr);
    auto tex = std::make_shared<TextureCubeMapArray>(shape);
