                                           ...
                                           bool isVirtualFile)
{
    ...
    while (getline(ss, line, '\n'))
    {
        ...
        if (line.find("#glow ") == line.find_first_not_of(" \t"))
        {
            ...
            std::copy(std::istream_iterator<std::string>(wordStream), std::istream_iterator<std::string>(),
            ...
            if (line.find('<') != std::string::npos)
            {
                ...
                else
                {
                    ...
                    if (virtualFileIt != sVirtualFiles.end())
                    {
                        ...
                        ++nextSrcIdx;
                        parseWithInclude(shader, parsedSrc, virtualFileIt->second, incIdx, nextSrcIdx, includes,
                                         "NONE_VIRTUAL", true);
                    }
                    else
                    {
...
void DefaultShaderParser::addIncludeResource(const std::string& file, const std::string& content)
{
    sIncludeResources[file] = content;
}

void DefaultShaderParser::addVirtualFile(const std::string& path, const std::string& content)
{
    sVirtualFiles[path] = content;
}
...
std::vector<std::string> DefaultShaderParser::getIncludePaths()
{
    return sIncludePaths;
}
...
bool DefaultShaderParser::resolveFile(const std::string& name, GLenum& shaderType, std::string& content, std::string& realFileName)
{
    ...
    // virtual file match
    auto virtualFileIt = sVirtualFiles.find(name);
    if (virtualFileIt != sVirtualFiles.end())
    {
        realFileName = "";
...
void DefaultShaderParser::CallbackShaderWriter::emitText(const std::string& content)
{
    mStream << content;
}
