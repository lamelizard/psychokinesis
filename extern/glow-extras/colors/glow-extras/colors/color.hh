...
public:
    ...
    color(glm::vec3 const& v, float a = 1.0f);
    color(glm::vec4 const& v);

    // factories
public:
    static color from_srgb(uint8_t r, uint8_t g, uint8_t b);
    static color from_srgb(glm::vec3 srgb); ///< 0..1^3
    static color from_srgb(uint32_t srgb);  ///< 0xRRGGBB

    static color from_srgba(uint8_t r, uint8_t g, uint8_t b, uint8_t a);
    static color from_srgba(glm::vec4 srgba); ///< 0..1^4
    static color from_srgba(uint32_t srgba);  ///< 0xRRGGBBAA

    static color from_hsv(glm::vec4 hsva);                            ///< 0..360 x 0..1 x 0..1 x 0..1
    static color from_hsv(glm::vec3 hsv, float a = 1.0f);             ///< 0..360 x 0..1 x 0..1 x 0..1
    static color from_hsv(float h, float s, float v, float a = 1.0f); ///< 0..360 x 0..1 x 0..1 x 0..1

    static color from_hsl(glm::vec4 hsla);                            ///< 0..360 x 0..1 x 0..1 x 0..1
    static color from_hsl(glm::vec3 hsl, float a = 1.0f);             ///< 0..360 x 0..1 x 0..1 x 0..1
    ...
    // conversions
public:
    glm::vec3 to_rgb() const;  ///< 0..1 x 0..1 x 0..1
    glm::vec4 to_rgba() const; ///< 0..1 x 0..1 x 0..1 x 0..1

    glm::vec3 to_srgb() const;  ///< 0..1 x 0..1 x 0..1
    glm::vec4 to_srgba() const; ///< 0..1 x 0..1 x 0..1 x 0..1

    glm::vec3 to_hsv() const;  ///< 0..360 x 0..1 x 0..1
    glm::vec4 to_hsva() const; ///< 0..360 x 0..1 x 0..1 x 0..1

    glm::vec3 to_hsl() const;  ///< 0..360 x 0..1 x 0..1
    glm::vec4 to_hsla() const; ///< 0..360 x 0..1 x 0..1 x 0..1

    glm::vec3 to_lab() const; ///< TODO

    glm::vec3 to_grayscale() const;   ///< 0..1 x 0..1 x 0..1
    glm::vec4 to_grayscale_a() const; ///< 0..1 x 0..1 x 0..1 x 0..1
...
inline color::color(const glm::vec3& v, float a) : r(v.r), g(v.g), b(v.b), a(a) {}
inline color::color(const glm::vec4& v) : r(v.r), g(v.g), b(v.b), a(v.a) {}
...
inline color color::from_srgb(glm::vec3 srgb)
{
    return {detail::srgb_to_linear(srgb.r), detail::srgb_to_linear(srgb.g), detail::srgb_to_linear(srgb.b), 1.0f};
}
...
inline color color::from_srgba(glm::vec4 srgba)
{
    return {detail::srgb_to_linear(srgba.r), detail::srgb_to_linear(srgba.g), detail::srgb_to_linear(srgba.b), srgba.a};
}
...
inline color color::from_hsv(glm::vec4 hsva)
{
    return from_hsv(hsva.x, hsva.y, hsva.z, hsva.a);
}

inline color color::from_hsv(glm::vec3 hsv, float a)
{
...
inline color color::from_hsl(glm::vec4 hsla)
{
    return from_hsl(hsla.x, hsla.y, hsla.z, hsla.a);
}

inline color color::from_hsl(glm::vec3 hsl, float a)
{
...
inline glm::vec3 color::to_rgb() const
{
    return {r, g, b};
}
inline glm::vec4 color::to_rgba() const
{
    return {r, g, b, a};
}

inline glm::vec3 color::to_srgb() const
{
    return {detail::linear_to_srgb(r), detail::linear_to_srgb(g), detail::linear_to_srgb(b)};
}
inline glm::vec4 color::to_srgba() const
{
    return {detail::linear_to_srgb(r), detail::linear_to_srgb(g), detail::linear_to_srgb(b), a};
}

inline glm::vec3 color::to_hsv() const
{
    return {hue(), saturation_hsv(), value()};
}
inline glm::vec4 color::to_hsva() const
{
    return {hue(), saturation_hsv(), value(), a};
}

inline glm::vec3 color::to_hsl() const
{
    return {hue(), saturation_hsl(), lightness()};
}
inline glm::vec4 color::to_hsla() const
{
    return {hue(), saturation_hsl(), lightness(), a};
}

inline glm::vec3 color::to_grayscale() const
{
    auto l = lightness();
    return {l, l, l};
}

inline glm::vec4 color::to_grayscale_a() const
{
    auto l = lightness();
    return {l, l, l, a};
}
...
inline float color::hue() const
{
    ...
    auto min = glm::min(r, glm::min(g, b));
...
inline float color::contrast_to(color c) const
{
    ...
    return (glm::max(l0, l1) + 0.05f) / (glm::min(l0, l1) + 0.05f);
}
...
{
    return {
        glm::clamp(r, min.r, max.r), //
        glm::clamp(g, min.g, max.g), //
        glm::clamp(b, min.b, max.b), //
        glm::clamp(a, min.a, max.a)  //
    };
}
...
{
    auto hsla = to_hsla();
    hsla.y = glm::clamp(hsla.y - amount, 0.0f, 1.0f);
...
{
    auto hsla = to_hsla();
    hsla.z = glm::clamp(hsla.z + amount, 0.0f, 1.0f);
    return from_hsl(hsla);
}
...
{
    auto hsla = to_hsla();
    auto new_h = glm::mod(hsla.x + hue, 360.0f);
