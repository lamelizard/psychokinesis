...
glm::mat4 GenericCamera::getMonoViewMatrix() const
{
    ...
    // assert(isApproxEqual(getRotationMatrix4() * getTranslationMatrix4(), m, mFarClippingPlane * 1e-5f));
    return m;
}
...
{
    if (distance2(_position, _target) > mFarClippingPlane * mFarClippingPlane)
